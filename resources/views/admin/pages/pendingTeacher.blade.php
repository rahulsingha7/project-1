...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800 mt-2">Pending Teachers</h1>
</div>
...
<script>
        $(document).ready(function() {
            ...
            function getAllUsers() {
                var str = ""
                $.ajax({
                    url: 'http://127.0.0.1:8000/api/show-pending-teacher',
                    type: 'GET',
                    dataType: "json",
                    success: function(result) {
                        if (result.status == 'success') {
            ...
            $(document).on('click', '#submit', function() {
                var id = $(this).attr('value');
                $.ajax({
                    url: `http://127.0.0.1:8000/api/approve-pending-teacher/${id}`,
                    type: 'POST',
                    dataType: "json",
                    data: {
                        activate: 1
         ...
         //Delete Student
          $(document).on('click','#submit2',function(){
            var id = $(this).attr('value');
            $.ajax({
                url: `http://127.0.0.1:8000/api/delete-pending-teacher/${id}`,
                type: 'POST',
                dataType: "json",
                success:function(result){
                    $("#myModaldel" + id).modal('hide');
                        if (result.status == 'success') {
                            Toastify({
                                text: result.message,
                                className: "success",
